...
export const saveVisitorRegistration = async (formData: VisitorFormData) => {
  try {
    ...
    if (formData.picture && typeof formData.picture !== "string") {
      ...
      pictureUrl = await uploadFile(formData.picture, "visitor-pictures", `${timestamp}-picture.png`);
    } else if (typeof formData.picture === "string") {
      pictureUrl = formData.picture;
    }

    // Upload signed policy PDF and use its URL as the signature_url
    if (formData.signedPolicyPdf) {
      const timestamp = Date.now();
      signatureUrl = await uploadFile(formData.signedPolicyPdf, "visitor-signatures", `${timestamp}-signed-policy.pdf`);
    ...
    const insertData = {
      ...
      user_id: null,
      starttime: formData.starttime || getISTString(),
